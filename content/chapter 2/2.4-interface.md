...
برگردیم به بحث اینترفیس, در مثال زیر ما یک نمونه اینترفیس را قرار دادیم :
...
برای اینکه مفهوم بالا را بهتر بفهمیم بزارید یک مثال ساده بزنیم. فرض کنید ما یک شی به نام animal که دارای یکسری رفتارها مانند: نفس کشیدن, راه رفتن و غیره را دارد داریم, و باید این رفتارها را به یک حیوان اختصاص دهیم تا بتوانیم رفتارهای آن حیوان را مشخص کنیم.
...
در مثال بالا ما یک اینترفیس تعریف کردیم که ۲ تا متد دارد. حالا بیاید یک متغیر از نوع اینترفیس animal درست کنیم و چاپ کنیم.
...
در مثال بالا وقتی اینترفیس را چاپ کردیم خروجی `nil` بود. توجه کنید مقدار پیش فرض اینترفیس `nil` هست.

## 2.4.1 پیاده سازی اینترفیس

در مثال بالا ما یک اینترفیس  animal تعریف کردیم که ۲ تا متد داشت. حالا قصد داریم یک شی (منظور ساختار در گو) به نام lion تعریف کنیم و متدهای اینترفیس animal را پیاده سازی کنیم. به مثال زیر توجه کنید:
...
در مثال بالا ما یک متغیر با تایپ animal تعریف کردیم :

```go
var a animal
```

سپس یک نمونه از ساختار lion را بهش اختصاص دادیم :

```go
a = lion{age: 10}
```

اختصاص یک نمونه از ساختار lion به متغیر a که از تایپ animal بود موفقیت آمیز بود, زیرا ما برای lion متدهای مورد مربوط animal را که breathe و walk بود پیاده سازی کردیم. این مفهوم کاملا شبیه به ducking typing هستش که در بالا گفتیم. یک شیر می تواند نفش بکشد و راه برود از این رو او یک حیوان است.

{{< hint info >}}

توجه کنید اگر شما متد جدیدی را اضافه یا کم کنید و همچنین اگر تغییری ایجاد کنید باید این تغییرات برروی اشیایی که با اینترفیس شما در ارتباط هستند نیز صورت بگیرد.

به عنوان مثال شما اگر به اینترفیس animal یک متد جدیدی اضافه کنید حتما باید برای ساختار lion اون رو پیاده سازی کنید.
{{< /hint >}}
